        ...
        async for message in websocket:
            ...
            if data["action"] == "doTask":
                ...
                if 'logout' in data:
                    ...
                    task = asyncio.create_task(websocket.send(json.dumps({"type":"logout", "user":user})))
                    await asyncio.wait([task])

                    # await asyncio.wait([websocket.send(json.dumps({"type": "logout", "user": user, "message": "user logged out" }))])
